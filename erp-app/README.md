...
一套可本机运行、可容器部署的外贸业务 ERP。覆盖客户、产品、供应商、多产品报价、销售合同、采购、出运、应收应付、资金、利润、月度分配、售后、审批、通知、附件和审计，并提供关联详情、版本保护修改、业务时间线、受控作废和多级审批。V2.0 新增跨页批量选择、所选记录 CSV 导出，以及客户阶段、产品启停和供应商准入状态的受控批量更新；所有更新校验公司归属、角色、数量和目标状态并写入审计。
...
测试覆盖未登录拦截、个人保存/默认视图、客户/产品/供应商、产品复制及供应商报价复制、CSV 模板/批量导入/重复拦截、多产品报价和费用合计、PI/合同打印、低毛利审批、职责分离、PI→SO、合同/采购/出运多级审批、申请撤回、待采购/待出运余量、应收应付生成时点、部分采购、超采拦截、部分出运、收付款、汇兑差额、财务冲正及余额恢复、售后、附件、利润、月度分配、分成审批与待付款统计和审计。
