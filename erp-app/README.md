...
一套可本机运行、可容器部署的外贸业务 ERP。覆盖客户、产品、供应商、多产品报价、销售合同、采购、出运、应收应付、资金、利润、月度分配、售后、审批、通知、附件和审计，并提供关联详情、版本保护修改、业务时间线、受控作废和多级审批。V3.0 完成服务端批量输出：客户、产品、供应商、PI、SO、PO、DO 和审批可跨页选择后导出 CSV、Excel 或批量打印并另存 PDF，PI/SO 会逐产品行展开；导出会再次校验公司与数据范围。客户、供应商、产品均提供标准 CSV 模板、逐行校验、文件内重复和数据库重复拦截及批量导入。
