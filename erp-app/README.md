...
一套可本机运行、可容器部署的外贸业务 ERP。覆盖客户、产品、供应商、报价、销售合同、采购、出运、应收应付、资金、利润、提成、售后、审批、通知、附件和审计，并提供关联详情、版本保护修改、业务时间线、受控作废和多级审批。V1.4 新增合同、采购、出运的生效审批，待采购/待出运池，“我的待办、我的申请、撤回”闭环，以及执行汇率/结算汇率差额与保留原始凭证的财务冲正；草稿或审批中的单据不会提前形成应收应付。
...
测试覆盖未登录拦截、客户/产品/供应商、低毛利审批、职责分离、PI→SO、合同/采购/出运多级审批、申请撤回、待采购/待出运余量、应收应付生成时点、部分采购、超采拦截、部分出运、收付款、汇兑差额、财务冲正及余额恢复、售后、附件、利润、提成和审计。
