...
一套可本机运行、可容器部署的外贸业务 ERP。覆盖客户、产品、供应商、多产品报价、销售合同、采购、出运、应收应付、资金、利润、月度分配、售后、审批、通知、附件和审计，并提供关联详情、版本保护修改、业务时间线、受控作废和多级审批。V2.8 新增按月份统计的经营看板，分别展示现金回款、新增签单、累计应收未收、现金支付、执行合同、客户资产、现金净额和待审批；同时加入严格只读的 Ask AI 操作助手，只解释航贸云页面和操作流程，不执行任何业务动作，也不回答经营预测、法律或决策问题。
