...
一套可本机运行、可容器部署的外贸业务 ERP。覆盖客户、产品、供应商、多产品报价、销售合同、采购、出运、应收应付、资金、利润、月度分配、售后、审批、通知、附件和审计，并提供关联详情、版本保护修改、业务时间线、受控作废和多级审批。V3.3 完成 MeetBase 功能矩阵：分成可直接上传客户确认凭证并批量发起；审批详情包含月度收入、毛利、分配、留存、冠军与 5K/10K 提醒，并可打印完整分配表；客户、供应商和产品支持原生 XLSX/CSV 导入；敏感银行账号按角色脱敏，付款账户禁止直接改写并强制走双人复核。
